...
def calculateHipPitchYaw(x1, y1, z1,  # 1 is the hip position
    ...
    alpha = trigo.angleBetween3Points3D(x2, y2, z2, x1, y1, z1, x3, y3,
                                        z3)  # calculate the angle between the ankle, the hip and the knee
...
def calculateHipPitch(angleHipPitchYaw,
    ...
    xNAO = math.cos(math.radians(angleHipPitchYaw)) * (
            math.sqrt(2) / 2) * radiusLength  # the x of the nao without pitch and roll, equals to k axis
